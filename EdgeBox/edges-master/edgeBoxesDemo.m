...
figure(3); bbGt('showRes',I,gtRes,dtRes(dtRes(:,6)==1,:));
title('green=matched gt  red=missed gt  dashed-green=matched detect');
%% 
figure(3),im(I);
for i= 1:20
   figure(3),drawBoxes(bbs(i,:)+[0 0 bbs(i,1) bbs(i,2) 0],'lineWidth',1);
end
...
boxesEval('data',data,'names',nm,'thrs',.6,'show',2,'cnts',10000);

% boxesEval('data',data,'names',nm,'thrs',.7:.05:1,'cnts',1000,'show',3);
%% Medial Axes Eva
if(~exist('boxes/VOCdevkit/','dir')), return; end
split='val'; data=boxesData('split',split);
nm='EdgeBoxes70'; opts.name=['boxes/' nm '-' split '.mat'];
